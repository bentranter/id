...
func main() {
	...
	serveMux.Handle("/auth/twitch/authorize", id.Authorize(provider))
	serveMux.Handle("/auth/twitch/callback", id.Callback(provider, "auth/restricted"))

	// Gorilla's Pat. Requires type assertion.
	p := pat.New()
	p.Get("/auth/twitch/authorize", id.Authorize(provider).(http.HandlerFunc))
	p.Get("/auth/twitch/callback", id.Callback(provider, "auth/restricted").(http.HandlerFunc))

	// Gorilla's Mux
	m := mux.NewRouter()
	m.Handle("/auth/twitch/authorize", id.Authorize(provider))
	m.Handle("/auth/twitch/callback", id.Callback(provider, "auth/restricted"))
